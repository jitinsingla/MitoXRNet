...
def Initialization(model_tag = 0, loss_tag=1):
    ...
    print()
    print(f'{device} founded, using it for training')
    # Create UNet mode
    if model_tag == 0:
        model_name = "UNet"
    ...
    return loss_fn, optimizer, model, train_loader, val_loader, device, model_name, loss_name

def train(loss_fn, optimizer, model, train_loader, val_loader, device, model_name, loss_name):
    
    plot_save_dir = "../Output/Plots"
    os.makedirs(plot_save_dir, exists_ok = True)
    checkpoint_path = f"../Outputs/Trained_Weights/Trained_model_{model_name}_{loss_name}"
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="1 = CombinedLoss, 0 = BCEWithLogitsLoss (default: 1)"
    )

    args = parser.parse_args()

    loss_fn, optimizer, model, train_loader, val_loader, device, model_name, loss_name = Initialization(
        model_tag=args.model_tag,
        loss_tag=args.loss_tag
    )

    print()
    print(f"---------- Starting training with Model = {model_name}, Loss = {loss_name} ----------")
    print()

    train(loss_fn, optimizer, model, train_loader, val_loader, device, model_name, loss_name)
